...
qubits the circuit really uses (prompts/12 step 4, the H0 rehearsal).  The yield proxy is
then the manual's 0.82 f with the f of gate S2D (per-edge CZ x readout of the used patch),
and the comparison is with validation/S2D.json.
...
def main():
    ...
    for twoB in (0, 2):
        ...
        if backend is not None:
            ...
            czs = [a["cz"] for a in an]
            f_model = float(np.mean([a["f"] for a in an]))
            predicted = YIELD_FACTOR * f_model          # manual Step 4.4, y ~ 0.82 f
            proxy_name = f"{YIELD_FACTOR} f, f from the {args.backend} calibration (gate S2D)"
        else:
            ...
            predicted = (1 - args.pro) ** n * f_model
            proxy_name = "(1-p_ro)^n f, f = <(1-p2)^CZ>"
        ...
        rows.append([f"B={twoB // 2}", len(circuits), shots, f"{cz:.0f}", f"{f_model:.3f}", f"{predicted:.3f}", f"{y:.3f}", f"{y / predicted:.2f}", len(B),
                     f"{res.ER - ref.E0:.1e}", f"{met['recall']:.2f}", met["false_positives"],
                     f"[{cert.weinstein[0]:.4f}, {cert.weinstein[1]:.4f}]", str(rej_all)])
        R.add(f"B={twoB // 2}: measured yield within a factor 3 of the proxy {proxy_name}",
              f"{y:.3f} vs {predicted:.3f} (ratio {y / predicted:.2f})",
              "ratio in [1/3, 3] (the proxy is a rough model)", predicted / 3 <= y <= 3 * predicted + 1e-9)
        ...
        R.data[f"B={twoB // 2}"] = dict(shots=shots, circuits=len(circuits), cz=cz, yield_=y, size=len(B), err=res.ER - ref.E0,
                                       ...
                                       f_model=f_model, predicted_yield=predicted,
...
{md_table(["sector", "circuits", "shots/circuit", "mean CZ", "f (model)", "predicted yield", "measured yield",
           "measured / predicted", "\\|B\\|", "E_R − E_0", "recall 99.9%", "fp", "Weinstein", "rejections"], rows)}

{R.criteria_table()}

Note: at 2x2 the sectors saturate (38 and 20 states), so the Ritz error is a consistency check only (manual
Step 9.1).  The prediction column is {rows[0][5]} / {rows[1][5]} = the proxy "{proxy_name}"; with --backend it is the
