...
Environment: Python 3.12.14, numpy 2.5.2, scipy 1.18.0, Linux-7.2.3-arch1-3-x86_64-with-glibc2.44, 12 CPUs, commit 1223e92, 2026-09-16 11:40:23 MDT.  Runtime 1299 s.  Nothing in this gate touches a QPU.
...
(pinned with --shots-by-rep allocation, 13692 shots in total, 1031 s; the pilot measured
0.033 s/shot at r=1, 0.076 s/shot at r=2, 0.102 s/shot at r=3,
...
random-string acceptance a = B=0 0.0093, B=1 0.0049
(section 3).  The accepted yield therefore cannot fall below (1 − f) a however deep the circuit is, so the
manual's y = 0.82 f describes the yield only while 0.82 f >> a, i.e. while f >> 0.011.
The column "0.82 f + (1 − f) a" adds that floor; it is a diagnostic added by this gate, not the
...
| sector | r | circuits | simulated yield | model yield | N/circuit (simulated) | N/sector (simulated) | N/circuit (model) | N/sector (model) |
|---|---|---|---|---|---|---|---|---|
...
| B=0 r=1 (663 CZ): simulated yield 0.149 vs the model 0.82 f = 0.103 | 1.443 | ratio in [0.33, 3] | PASS |
| B=0 r=2 (1308 CZ): simulated yield 0.038 vs the model 0.82 f = 0.013 | 3.01 | ratio in [0.33, 3] | FAIL |
| B=0 r=3 (1910 CZ): simulated yield 0.017 vs the model 0.82 f = 0.002 | 8.284 | ratio in [0.33, 3] | FAIL |
| B=1 r=1 (663 CZ): simulated yield 0.135 vs the model 0.82 f = 0.100 | 1.36 | ratio in [0.33, 3] | PASS |
| B=1 r=2 (1296 CZ): simulated yield 0.029 vs the model 0.82 f = 0.014 | 2.074 | ratio in [0.33, 3] | PASS |
| B=1 r=3 (1944 CZ): simulated yield 0.011 vs the model 0.82 f = 0.002 | 5.573 | ratio in [0.33, 3] | FAIL |
...
| pytest -q tests | 31 passed in 100.43s (0:01:40) | all pass | PASS |
